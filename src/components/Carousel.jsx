...
export default function Carousel({ children, itemWidth, gap, previousButton, nextButton, pip }) {
	...
	useEffect(() => {
		const handleResize = () => {
			setPipsCount(Math.ceil((children.length * (itemWidth + gap) - gap) / el.current.offsetWidth));
		}
		setPipsCount(Math.ceil((children.length * (itemWidth + gap) - gap) / el.current.offsetWidth));
		setPosition(0);
		window.addEventListener('resize', handleResize);
		return () => window.removeEventListener('resize', handleResize);
	}, [itemWidth, gap, children]);
		...
		<div className="Carousel" ref={el}>
			<div className="Carousel__wrapper">
				<ul
					...
					onTouchMove={swipe}
				>
					{children.map((item, idx) => <li key={idx} className="Carousel__item">{item}</li>)}
				</ul>
			</div>
			...
			{
				nextButton && <span
					className={position <= -(children.length * (itemWidth + gap) - gap) + el?.current?.offsetWidth ? 'Carousel__button-disabled' : ''}
					onClick={() => slide(-(itemWidth + gap))}
				>
					{nextButton}
				</span>
